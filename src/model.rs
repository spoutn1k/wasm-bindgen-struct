...
impl Field {
    fn to_tokens_with_global(&self, global: &Struct) -> TokenStream {
        ...
        let final_ =
            (!*structural && (*final_global || *final_)).then(|| quote! { #[wasm_bindgen(final)] });
...
impl Method {
    ...
    fn to_tokens_with_global(&self, ty: &syn::Type, options: &ImplAttributes) -> TokenStream {
        let ImplAttributes {
            dbg: _,
            final_: final_global,
            js_name: js_class,
        ...
        let method = (!self.is_static()).then(|| quote! { #[wasm_bindgen(method)] });

        let constructor = constructor.then(|| quote! { #[wasm_bindgen(constructor)] });

        let final_ =
            ((*final_global && !structural) || *final_).then(|| quote! { #[wasm_bindgen(final)] });
        ...
        let indexing_getter = indexing_getter.then(|| quote! { #[wasm_bindgen(indexing_getter)] });

        let indexing_setter = indexing_setter.then(|| quote! { #[wasm_bindgen(indexing_setter)] });
        ...
        let (catch, inner_sig) = {
            ...
            let catch =
                is_result_from_return_ty(&sig.output).then(|| quote! { #[wasm_bindgen(catch)] });
